...
## Python seed가 드러낸 것: 디렉터리 이름이 분석 여부를 정한다

`DEFAULT_SOURCE_DIRS = ["src", "lib", "app", "packages", "python"]`이고 탐색은 **재귀적이지
않다**(`core/project.py:17`, `:95`). `multi_component` seed는 소스를
`components/*/src/`에 두는데, `components`는 목록에 없다.
...
**분석된 것이 0개일 때 결과가 PASS다.** 깨끗한 코드와 구분되지 않는다. 이 마일스톤이
반복해서 부딪치는 "미실행 ≠ 통과"가 이번에는 **소스 탐색 수준**에서 나타난 것이다.

**기록했을 뿐 승인한 것이 아니다.** "범위에 아무것도 없음"과 "잘못된 것이 없음"을 가르는
scope 모델은 #207의 몫이고(R05), 그것을 corpus 변경 안에서 바꾸는 것은 scope 의미론을
재설계하는 일이다. 그래서 seed는 **오늘 실제로 일어나는 일**을 expectation에 적고,
테스트가 그것을 고정한다 — #207이 고치면 이 테스트가 변화를 알린다.

간극은 **기본값에 있지 메커니즘에 있지 않다.** 설정으로 지정하면 찾는다.

`multi_component` 안에서 `alpha`는 `beta`의 대조군이다 — 거기 붙는 finding은 두 번째 탐지가
아니라 **오귀속**이다. 테스트가 그것도 고정한다.
...
|Python data fixture (정상 대조군 1 + 결함 seed 1)|2|

...
> **실측 결과 2 (2026-08-30, diskmap 전환).** `-xunitxml` 은 **QtTest 바이너리에만** 의미가
> 있다. `diskmap` 의 기존 테스트 5개는 자체 `main()` 을 쓰는 수제 assert 라 그 인자를 무시하고
> `All checks passed` 만 출력했다 — XML 이 하나도 나오지 않는다. 그대로 두면 `parse_qtest_xunit`
> 이 빈 목록을 돌려주고 엔진은 **"테스트 0건"** 으로 판단한다. 실제로는 5개가 전부 통과한
> 상태에서 게이트가 그렇게 보고한다.
>
> 그래서 qmake 경로는 **`make check` 의 전사(transcript)를 기준으로** 읽는다. make 는 각 명령을
> 실행 전에 그대로 출력하므로 그 줄이 테스트 이름이 되고, 실패하면 `Error N` 을 찍고 멈추므로
> 실패를 그 테스트에 귀속시킬 수 있다. 귀속되지 않은 비정상 종료는 **마지막으로 시작된
> 테스트**의 실패로 본다 — 그 뒤 것들은 아예 실행되지 않았기 때문이다.
>
> **XML 을 우선하면 안 된다.** 실물 qmake 프로젝트는 QtTest 바이너리와 자체 `main()` 테스트를
> 섞어 갖는다(`diskmap` 이 정확히 그렇다: 코어 5개는 수제 assert, 위젯 1개는 QtTest). XML 을
> 먼저 보면 QtTest 바이너리만 보고되고 **나머지는 조용히 사라진다** — 아무도 보지 않은 테스트
> 위에 초록불이 서는 모양이다. 전사가 실행된 바이너리 전부를 알려주므로 그것이 기준이다.
>
> **실측 결과 1 (2026-08-30, 픽스처 검증).** `TESTARGS=-xunitxml` 는 QtTest 픽스처에서 설계대로
> 동작했다. 처음에는 XML 을 그대로 읽어 **테스트 함수**를 세었고, 그래서 같은 픽스처가 CMake 에서
> 1건(바이너리), qmake 에서 4건(함수)이 됐다. 실측 결과 2 를 반영하면서 **전사를 기준으로 삼아
> 바이너리 단위로 통일**했으므로 이 차이는 없어졌다 — 두 백엔드의 카운트는 이제 같은 것을 센다.
> QtTest 의 함수 단위 정보는 버리지 않는다. `make` 는 첫 실패에서 멈추므로 XML 에 담긴 실패는
> 그때 실행 중이던 바이너리의 것이고, 그 이름과 메시지를 해당 바이너리의 실패 메시지에 붙인다.
...
**`project.cpp_external_build_dirs` 는 `build` 와 `test` 에서 무시된다.** 이 설정은 "moc 가
필요해 ici 가 직접 빌드할 수 없는 소스를 링크 대상에서만 뺀다" 는 뜻이었고, 어댑터가 생기면
그 전제가 사라진다. CMake·qmake 가 전부 빌드하므로 gui 소스도 링크되고 커버리지 대상에 포함된다.

이것이 성립하려면 **바이너리를 만드는 세 엔진이 전부** 어댑터를 써야 한다. `sanitize` 를 범위
밖에 두었던 원래 판단으로는 성립하지 않았다 — `loglens` 에서 이 설정을 지웠더니 `sanitize` 가
링크에 실패해 `NOT_RUN` 으로 스위트를 `ERROR` 로 만들었다. §8 을 본다.
...
### 4.3 `ici/viewer` → CMake 어댑터로 전환됨 (설계 수정, 2026-08-30)

**이 절의 원래 계획은 틀렸다.** "viewer 는 기존 g++ 경로를 유지한다" 고 적었으나, `viewer/` 는
자기 자신이 프로젝트 루트이고 `viewer/CMakeLists.txt` 가 바로 그 루트에 있다. 즉 `viewer` 에서
`ici verify` 를 돌리면 §3.2 의 선택 규칙에 따라 어댑터 경로로 간다. 유지할 선택지가 없었다.

실제로 확인한 결과 그대로였다 — 전환 직후 `viewer` 는 테스트 0건으로 FAIL 했다. 루트
`CMakeLists.txt` 에 `enable_testing()` 과 테스트 타깃이 없었기 때문이다. 그래서 `viewer` 를
어댑터 경로로 **정식 전환**한다: 파싱·모델 코드를 `icirv_core` 라이브러리로 빼고, 테스트 3개를
...
넘어가 g++ 경로가 아무 데서도 검증되지 않는 상태를 막는다.
...
- ~~**`sanitize` 엔진의 어댑터 전환**~~ — **범위에 들어왔다 (실측, 2026-08-30).** 선택 사항이
  아니었다. `sanitize` 는 `tests/**/*.cpp` 를 각각 plain g++ 로 컴파일하므로, Qt 테스트를
  `tests/` 에 두는 순간 `<QAbstractItemModelTester>` 를 찾지 못해 컴파일이 깨진다. `loglens`
  에서 실제로 그렇게 됐다. **"Qt 테스트를 `tests/` 안에 두고 통과시킨다" 는 이 작업의 목표가
  `sanitize` 전환을 전제로 한다** — 옮기지 않으면 목표 자체가 불가능하다.

  전환하면서 configure 옵션이 필요해졌다. 세 엔진이 원하는 빌드가 서로 다르기 때문이다.

  | 엔진 | 플래그 | shadow 디렉터리 |
  |---|---|---|
  | `build` | 없음 — 릴리스 산출물을 계측하면 안 된다 | `build/ici-<backend>-build` |
  | `test` | `--coverage` | `build/ici-<backend>` |
  | `sanitize` | `-fsanitize=address,undefined` | `build/ici-<backend>-asan` |

  디렉터리를 나누는 이유는 하나의 트리를 공유하면 **엔진이 돌 때마다 상대의 오브젝트를 다른
  플래그로 다시 빌드**하기 때문이다. `build` 에서 커버리지를 끈 것도 이 과정에서 발견했다 —
  원래 구현은 릴리스 산출물에도 `--coverage` 를 주입하고 있었다.

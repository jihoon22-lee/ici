...
**그러나 `sanitize` 는 계속 이 설정을 따른다** (실측, 2026-08-30). §8 이 `sanitize` 를 이번
범위에서 뺐으므로 그 엔진은 여전히 `g++` 로 직접 링크하고, 그 경로는 `Q_OBJECT` 의 vtable 을
만들 수 없다. `loglens` 에서 이 설정을 지웠더니 `sanitize` 가 링크에 실패해 `NOT_RUN` 으로
스위트를 `ERROR` 로 만들었다. **즉 이 설정의 적용 여부는 "경로별" 이 아니라 "엔진별" 이다.**
사용자 문서는 그렇게 적어야 한다 — 경로별이라고 적으면 Qt 프로젝트가 전환 후 설정을 지웠다가
게이트가 막힌다.

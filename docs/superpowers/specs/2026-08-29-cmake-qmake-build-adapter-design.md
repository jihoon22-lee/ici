...
> 상태에서 게이트가 그렇게 보고한다.
>
> 그래서 qmake 경로는 두 단계로 읽는다. 구조화된 XML 이 있으면 그것을, 없으면 **`make check`
> 의 전사(transcript)** 를 읽는다. make 는 각 명령을 실행 전에 그대로 출력하므로 그 줄이
> 테스트 이름이 되고, 실패하면 `Error N` 을 찍고 멈추므로 실패를 그 테스트에 귀속시킬 수 있다.
> 귀속되지 않은 비정상 종료는 **마지막으로 시작된 테스트**의 실패로 본다 — 그 뒤 것들은 아예
> 실행되지 않았기 때문이다.
>
> **실측 결과 1 (2026-08-30, 픽스처 검증).** `TESTARGS=-xunitxml` 는 QtTest 픽스처에서 설계대로
> 동작했다. 다만
> **두 백엔드의 결과 단위가 다르다는 것이 드러났다.** CTest 는 테스트 **바이너리** 하나를 1건으로
> 보고하고, QtTest 는 그 안의 테스트 **함수**를 각각 보고한다(`initTestCase`/`cleanupTestCase`
> 포함). 같은 픽스처가 CMake 에서 1건, qmake 에서 4건이 됐다.
>
> 이것은 TEM 의 통과/전체 카운트에 직접 들어가므로 **두 경로의 점수를 서로 비교할 수 없다.**
> 어느 쪽으로 정규화하지는 않는다 — 각 빌드 시스템이 "테스트 하나" 로 여기는 단위를 그대로
> 쓰는 편이 정직하고, 억지로 맞추면 어느 쪽에서도 사실이 아닌 수가 된다. 대신 이 차이를
> `docs/user-guide.md` 에 적어 읽는 사람이 카운트를 오해하지 않게 한다.

...
예산은 실측의 약 3배입니다. 처음 그 배수를 고른 근거는 "공유 runner가 기준 워크스테이션보다
몇 배 느릴 것"이라는 **추정**이었는데, 첫 CI 실행이 그 추정을 반증했습니다. 실제로는 훨씬
가깝습니다.

| 단계 | 기준 워크스테이션 | GitHub runner | 배수 | 예산 사용률 |
|---|---|---|---|---|
| `console-default` | 11.4s | 14.2s | 1.25× | 41% |
| `console-verbose` | 51.8s | 54.3s | 1.05× | 35% |
| `html` | 15.5s | 18.8s | 1.21× | 38% |
| `json` | 14.5s | 19.7s | 1.36× | 44% |
| `sarif` | 12.8s | 15.1s | 1.18× | 38% |

(GitHub `ubuntu-latest`, CPython 3.10.21, 10만 finding. 벤치마크 단계 전체는 122초입니다.)

runner는 1.05~1.36배 느릴 뿐이므로, 3배 예산의 근거는 "느린 기계를 감당한다"가 아니라
**"리포터가 finding 수에 대해 초선형이 되는 회귀만 잡고 그보다 작은 변동은 보고만 한다"**입니다.
그 목적에는 3배가 여전히 맞습니다 — 실행별 변동은 예산의 절반 아래에서 움직이고, 알고리즘이
바뀌는 종류의 회귀는 배수로 넘어갑니다.

#### 브라우저가 실제로 치르는 비용 (로컬 실측)

위 예산은 리포트를 **생성하는** 비용입니다. 브라우저가 그것을 **여는** 비용은 다른 문제이고,
`scripts/benchmark_browser.py`가 헤드리스 Chrome으로 실측합니다.

```bash
uv run python scripts/benchmark_browser.py --runs 3 --json browser.json
```

CI에는 넣지 않습니다. 브라우저는 큰 의존성이고 매 PR에 몇 분을 더하는데 이 숫자는 천천히
움직입니다. 대신 CI는 **브라우저 비용을 결정하는 입력**을 매 실행 고정합니다 — 초기 렌더 행 수,
inline JSON 바이트, HTML 총 바이트가 `report_benchmark.json`의 `browser_payload`에 들어갑니다.
그 셋이 회귀하면 브라우저 비용도 회귀한 것입니다. HTML 리포터의 모양이 바뀌면 이 스크립트를
손으로 다시 돌리고 아래 숫자를 갱신합니다.

2026-09-07 실측 (Chrome headless, Linux x86-64, 10만 finding, 3회 중 최선):

| 항목 | 값 |
|---|---|
| 열기까지 (startup + parse + layout) | **9.4초** |
| 최대 단일 프로세스 RSS (renderer) | **653 MiB** |
| HTML 총 크기 | 34.0 MB |
| inline JSON | **33.9 MB (파일의 99.7%)** |
| 서버 렌더 행 수 | 50 |

메모리는 **가장 큰 단일 프로세스**를 씁니다. Chrome은 browser·zygote·renderer로 갈라지고 각
프로세스의 RSS가 공유 매핑을 중복해서 세므로, 트리 합계(1,780 MiB)는 공유분만큼 과대계상입니다.
DOM은 renderer에 있으므로 "이 페이지가 얼마나 들었나"의 정직한 답은 renderer 쪽입니다. 트리
합계는 상한으로만 기록합니다.

**여기서 드러난 것이 중요합니다.** 초기 DOM을 50행으로 제한하는 것은 **렌더링을 보호하지 로딩을
보호하지 않습니다.** 파일의 99.7%가 inline JSON이고, 브라우저는 첫 행을 그리기 전에 그 34 MB를
전부 내려받아 파싱해야 합니다. 10만 finding 리포트를 더 빠르게 열려면 줄여야 할 것은 초기 행
수가 아니라 그 JSON입니다.

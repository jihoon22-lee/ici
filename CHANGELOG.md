...
- **`Q_OBJECT` 클래스를 단위 테스트할 수 있습니다 (`test`)**: C++ 테스트를 `g++ -std=c++17` 로 직접 컴파일·링크하는 대신 프로젝트의 CMake/qmake 정의에 위임합니다. moc 가 빌드 시스템 쪽에서 돌므로 `Q_OBJECT` 클래스가 더 이상 vtable 미해결로 링크에 실패하지 않고, `-std` 고정도 사라져 C++20/23 프로젝트를 검증할 수 있습니다.
  ...
  - `build` 와 `test` 는 어댑터 경로에서 `project.cpp_external_build_dirs` 를 무시합니다. 이 설정은 ici 가 moc 를 돌리지 못한다는 전제 위에 있었고, 어댑터가 그 전제를 없앱니다. **`sanitize` 는 어댑터로 옮기지 않아 계속 이 설정을 따릅니다** — Qt 위젯이 있는 프로젝트에서 지우면 `sanitize` 가 링크에 실패해 게이트를 막습니다.

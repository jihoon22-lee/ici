...
두 가지 하드코딩이 새 경로에서 빠졌고, **같은 실수의 다른 옷**입니다 — **아무도 넣지 않았는데
답에 들어온 위치**:

- `core/env.py`의 `get_nas_cpp_lib_dir`는 **한 부서의** C++ 라이브러리를 **한 버전에 고정해서**
  ici 안에 컴파일해 넣습니다.
- `find_project_executable`은 프로젝트가 말했든 안 했든 `.venv`를 봅니다 — **ici 자신의
  인터프리터로 끝나는 추측 사슬의 첫 고리**입니다.

- **아무것도 선언하지 않은 component는 후보가 0개입니다.** resolver에 **되돌아갈 것이
  없습니다.** #204의 첫 인수 기준은 *"대체하지 않기를 기억해서"*가 아니라 **대체할 것이 없어서**
  달성됩니다.
- analyzer는 **bundle이거나 명시된 external**이고 **PATH는 후보가 아닙니다** — PATH에서 주운
  linter는 결과를 *그 기계에 또 무엇이 깔려 있는지*에 달리게 만듭니다.
- **migration 경고가 썩지 않습니다.** 경고가 지고 있는 값은 *"당신 빌드가 이것에 의존하고 대신
  이렇게 쓰라"*인데 **설명하는 코드가 옮겨지면 거짓이 됩니다.** 각 항목이 파일과 본문 표식을 들고
  `stale()`이 그것을 다시 읽어, 없어진 코드를 설명하는 경고는 **테스트를 실패시킵니다.**
- 하드코딩 부재 검사는 **구문 트리**로 합니다. 처음 쓴 검사는 소스를 grep 했다가 *"새 경로가
  무엇을 피하는지 설명하는 docstring"*에 걸려 실패했습니다 — **산문을 읽는 검사는 산문만 맞고
  코드는 틀린 모듈도 통과시킵니다.**

**live engine cutover는 이 PR에서 제외했고, 이유를 측정했습니다.** `_resolve_python`을 새
resolver로 갈아끼우면서 자동 `.venv` 우선선택도 함께 빼면 — 작업 7이 요구하는 그대로 —
**ici 자신의 `test` 게이트가 깨집니다.** ici의 `ici.toml`은 인터프리터를 선언하지 않고
(`[engines.test]`에 `python` 키가 없습니다) 전적으로 자동 `.venv` 탐색에 의존하므로, 새 규칙
아래에서 후보 0개가 됩니다 — **규칙이 옳게 동작한 결과**이지만 설정이 먼저 따라오지 않으면 ici가
자기 자신을 검증하지 못합니다. 순서가 있습니다: **설정이 인터프리터를 선언할 수 있게 된 뒤에**
engine을 옮기고, 그 연결은 [#210](https://github.com/jihoon22-lee/ici/issues/210)입니다.
현행 fallback은 이미 테스트로 고정돼 있어 옮기는 PR의 diff가 **바뀐 동작**을 보여줍니다.

### 추가 — 실제 프로세스로 확인한 환경 분리 (WP06 PR B, [#204](https://github.com/jihoon22-lee/ici/issues/204))

**기존 배포 경로 동작 변경 없음.**

PR A의 규칙은 가짜 실행 파일로 시험했고, 그래서 **증명할 수 없는 것이 하나 남았습니다** —
`.venv/bin/python`으로 실행하는 것과 그것이 가리키는 파일로 실행하는 것이 **사실로** 다른가.
진짜 venv를 만들어 쟀습니다:

```
.venv/bin/python  → sys.prefix = <project>/.venv, 프로젝트가 선언한 모듈 import 됨
그 realpath       → sys.prefix = /usr,            import 안 됨
```

다섯 번째 인수 기준이 요구하는 것이고, **단위 테스트를 아무리 늘려도 할 수 없는 주장**입니다.
venv의 site-packages에 모듈을 **직접 써넣어** 만들므로 네트워크가 필요 없습니다.

- `launch.py`는 **상속하지 않습니다** — `subprocess`에 스냅샷 매핑만 줍니다. 상속된
  `PYTHONPATH`는 보고하는 것과 **다른 트리를 분석**하게 만드는 바로 그 경로입니다.
- **timeout과 출력 폭주를 결과의 사실로** 돌려줍니다. 예외로 던지면 첫 `except`에서 "없음"과
  합쳐지고, PR A가 셋으로 나눈 구분이 다시 뭉개집니다.
- 이것이 PR A의 고리를 닫습니다 — resolver가 인자로 요구한 probe가 이제 실물이고, **도구를 어느
...
- **`init`은 제안하고 바꾸지 않습니다.** 읽는 함수와 쓰는 함수를 나눴습니다 — `propose()`에는
  **쓸 방법이 아예 없고**, 쓰는 것은 별개 호출이며 기존 파일이 있으면 거절합니다. 이 분리가
  *"init은 아무것도 바꾸지 않는다"*를 약속이 아니라 **확인 가능한 사실**로 만듭니다.
  자동 overwrite·패키지 설치·source·qmake configure·테스트 실행은 하지 않습니다.
- **모르는 것은 모른다고 씁니다.** 빌드 파일이 둘 이상이면 고르지 않고 `# UNDECIDED:` 로
  남깁니다. 디렉터리 순서로 정해진 빌드는 **아무도 결정하지 않은 빌드**이고, 거기서 나온 분석은
  누군가 결정한 것과 **똑같이 생겼습니다**(SPEC-01 §6). 빌드 파일이 하나뿐이면 선택이 아니라
  유일한 후보라 적습니다.
- **남의 도구 설정은 가리키기만 합니다.** `pyproject.toml`·`ruff.toml`·`mypy.ini`가 있으면 있다는
  사실과 위치만 적고 **설정 값은 한 줄도 옮기지 않습니다**(SPEC-01 §7). 복사본은 두 번째
  진실이 되고 원본을 처음 고치는 순간부터 갈라집니다.
- **탐색이 하지 않는 네 가지**: `[component]` 파일을 workspace로 승격하지 않고, root가 등록하지
  않은 하위 `ici.toml`을 병합하지 않고, 중첩 workspace를 흡수하지 않고, **VCS root 위로** 가지
  않습니다. 각각이 엉뚱한 것을 분석하게 되는 경로입니다 — 승격하면 *"workspace가 통과했나"*가
  **어느 디렉터리에 서 있었는지**에 달립니다. 하위 디렉터리에서 실행해도 같은 scope가 나오는
  것을 테스트가 고정합니다.
- 찾지 못하면 **무엇을 봤는지** 말합니다. 등록되지 않은 component 파일을 지나쳤으면 그 사실을
  따로 말합니다 — *"여기 설정이 없다"*와 *"그 설정이 어디에도 등록돼 있지 않다"*는 고치는 방법이
  다릅니다.
- **migration 보고서는 바꾸지 않고 설명합니다.** 현행 로더가 읽는 파일들을 같은 순서로 읽고,
  둘 이상이 정한 키마다 **이긴 쪽과 진 쪽을 같이** 댑니다:
  `engines.lint.required: dev.toml=False overrides project ici.toml=True`.
  이 목록이 곧 migration이고, **현행 로더의 출력으로는 얻을 수 없습니다** — `_deep_merge` 뒤에는
  누가 이겼는지가 남지 않습니다. 그중 게이트 판정을 바꿀 수 있는 것(`required`/`enabled`/
  `mode`/`threshold`)이 있는지는 따로 말합니다. **변환은 하지 않습니다** — 보고서 안에서 하면
  작업 7이 금지한 *"새 경로에 조용히 섞기"*가 됩니다.
- **filesystem side-effect 검사**를 추가했습니다: 읽기·탐색·실패한 읽기·제안이 **파일을 하나도
  건드리지 않고**, 쓰기는 **대상 파일만** 건드립니다(경로·크기·mtime 전수 비교).
- 문서: [`config-usage.md`](docs/design/ici-next/config-usage.md).

CLI 명령 연결과 `Layer.CLI` 적용은 [#210](https://github.com/jihoon22-lee/ici/issues/210),
기존 로더 교체는 [WP27 #225](https://github.com/jihoon22-lee/ici/issues/225)입니다.
...
- **smoke 자신의 첫 판이 세 군데 틀렸습니다.** `no-writes`는 앞선 케이스들이 이미 더럽힌
  ...
  이제 실행이 성공했는지를 먼저 확인합니다. 세 번째는 CI가 잡았습니다 — help 검사가 **색이
  입혀진 바이트를 그대로 비교**하고 있어서, rich가 색을 켜는 러너에서는 `Usage: `와 `ici`
  사이에 escape가 끼어들어 **번들이 멀쩡한데도 10개 중 7개가 실패**했습니다. 이제 escape를
  걷어내고 텍스트를 비교하며, 실패 메시지가 **실제로 본 줄을 인용**합니다. 각 가드는 고의로
  깨뜨려 **실제로 실패하는 것을 확인**한 뒤에 넣었습니다.
...
직전 변경이 호출 기록 harness를 만들었지만 **그것으로 ici를 검증하는 테스트는 없었습니다.**
도구만 있고 그 도구가 가능케 하는 검증이 없는 상태였습니다. `run_process` —
ici가 무언가를 spawn하는 **유일한 지점** — 에 대해 실측으로 고정했습니다.
한 지점을 시험하므로 새 엔진이 이 보장을 자동으로 물려받습니다.

- **R01의 "셸 초기화 파일 source 금지"가 처음으로 실행 가능한 증명을 얻었습니다.**
  그동안은 아키텍처 주장이었을 뿐입니다. HOME에 `.bashrc`·`.profile`·`.bash_profile`·`.zshrc`
  넷을 심어도 자식 환경에 아무것도 주입되지 않습니다(어느 프로파일을 읽는지는 셸과 대화형
  여부에 달려 있어 넷 다 심습니다).
- **셸이 끼지 않음을 메타문자로 증명합니다.** `$HOME` `*` `` `id` `` `a;b` `$(whoami)`가
  문자 그대로 도착합니다. 음성 대조로 확인했습니다 — `shell=True`였다면 `$HOME`이 `/root`로,
  `*`가 `bin`으로 확장되고 `a;b`가 명령을 분리해 도구는 `('/root', 'bin', 'a')`만 받습니다.
- **환경 보존**(R02): `env=`는 상속 환경을 덮지 않고 추가합니다. `replace_env=True` 후
  자식이 보는 키는 `{PATH, LC_CTYPE}` 이하이고, `LC_CTYPE`은 인터프리터가 붙이는 것이라
  이름을 박아 "어느 정도 새는 것"을 허용하지 않습니다.
- **fail과 blocked를 구분합니다.** 도구가 돌고 실패하면 자기 종료 코드와 출력을 냅니다.
  도구가 없으면 `rc=-1` + `"Failed to execute"`이고 예외를 던지지 않습니다.
- **blocked와 시그널 사망도 구분합니다.** SIGKILL은 `rc=-9`에 빈 stderr입니다. 다만
  **SIGHUP 사망도 `rc=-1`이라 종료 코드만으로는 구분되지 않고 stderr 표지가 실제 구분자**임을
  테스트로 고정했습니다 — 리팩터가 두 경로를 합치지 못하도록.
- **실행 횟수**와 **출력 잘림 보고**도 함께 고정했습니다.
...
- **네이티브 분석의 증거 등급을 분리했습니다.** C++ 함수 경계는 가능하면 컴파일러가 정하고,
  그 안의 CC/nesting 값은 언제나 `bounded-cpp-tokens` lexical estimate 입니다. 경계 출처와
  metric 신뢰도를 각각 따로 보고하므로, 어떤 숫자가 측정값이고 어떤 숫자가 추정값인지
  리포트만 보고 구분할 수 있습니다.
- **gcov JSON 커버리지를 1급 증거로 다룹니다.** 형식/도구 버전을 검증하고, 함수 line/column
  기하와 demangle 이름을 보존하며, 기대 production source 집합 전체를 확인합니다. JSON 을
  광고하지 않는 legacy gcov 만 문서화된 저정밀 text fallback 을 씁니다 — 판정 불가 probe 나
  깨진 JSON 은 조용히 fallback 하지 않고 실패합니다.
- **SARIF 에 수정 제안(`fixes`)을 실었습니다.** 한 제안이 두 파일을 고치면 두 개의
  `artifactChanges` 를 가진 **하나의** fix 로 나갑니다. `fixes` 는 fingerprint 에서
  의도적으로 제외했습니다 — 도구를 업그레이드해 제안 문구가 바뀌어도 기존 finding 이
  새 finding 으로 보이지 않습니다.
- **dead 엔진의 실제 오탐을 고쳤습니다.** 링커 `--gc-sections` 증거를 링크 대상별로 합집합
  하던 것을 교집합으로 바꿨습니다. 실행 파일 A 가 버린 헬퍼를 B 가 호출하면 이전 버전은
  제거 가능으로 보고했고, 지우면 B 의 빌드가 깨졌습니다. 실행 파일이 여럿인 프로젝트
  (ici 자신의 `viewer/` 포함)가 영향을 받았습니다.

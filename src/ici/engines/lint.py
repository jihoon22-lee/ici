    ...
    def _cpp_finding_category(diagnostic: CppDiagnostic) -> FindingCategory:
        ...
        if family in {"clang-analyzer", "clang-tidy"}:
            if rule.startswith(_CLANG_SECURITY_PREFIXES) or (
                family == "clang-tidy"
                and (
                    rule.startswith(_CLANG_TIDY_SECURITY_PREFIXES)
                    or rule in _CLANG_TIDY_SECURITY_RULES
                )
            ):
        ...
        if rule in _CLAZY_RESOURCE_RULES or _matches_rule_stem(rule, _CLAZY_RESOURCE_STEMS):
            return FindingCategory.RESOURCE
        if rule in _CLAZY_COMPATIBILITY_RULES or _matches_rule_stem(
            rule, _CLAZY_COMPATIBILITY_STEMS
        ):
            return FindingCategory.COMPATIBILITY
